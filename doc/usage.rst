...
Stechec2 uses `CMake <https://cmake.org/>`_ as its build system.
...
Stechec2, how, etc. At the root of the source directory, run:

.. code-block:: bash

  cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug -DBUILD_TESTING=ON -DBUILD_GAMES=ON

This will configure the build system to build Stechec2 with the bundled games
and tests. All compilation artifacts will go into the ``build`` directory.
